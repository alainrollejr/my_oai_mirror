  ...
  for (pdu_session_to_setup_t *i = bearerCxt->pduSession; i < bearerCxt->pduSession + bearerCxt->numPDUSessions; i++) {
    ...
    for (DRB_nGRAN_to_setup_t *j = i->DRBnGRanList; j < i->DRBnGRanList + i->numDRB2Setup; j++) {
      ...
      if (j->dRB_QoS.gbr_qos_flow_info) {
        asn_long2INTEGER(&(drb_QoS_gbr_info->guaranteedFlowBitRateDownlink), (j->dRB_QoS.gbr_qos_flow_info->guar_flow_bit_rate_dl));
        asn_long2INTEGER(&(drb_QoS_gbr_info->guaranteedFlowBitRateUplink), (j->dRB_QoS.gbr_qos_flow_info->guar_flow_bit_rate_ul));
        asn_long2INTEGER(&(drb_QoS_gbr_info->maxFlowBitRateDownlink), (j->dRB_QoS.gbr_qos_flow_info->max_flow_bit_rate_dl));
        asn_long2INTEGER(&(drb_QoS_gbr_info->maxFlowBitRateUplink), (j->dRB_QoS.gbr_qos_flow_info->max_flow_bit_rate_ul));
      }
