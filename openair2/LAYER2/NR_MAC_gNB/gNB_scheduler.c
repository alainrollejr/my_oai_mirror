...
void gNB_dlsch_ulsch_scheduler(module_id_t module_idP, frame_t frame, sub_frame_t slot, NR_Sched_Rsp_t *sched_info)
{
  ...
  AssertFatal(*scc->ssbSubcarrierSpacing >= 0
              && *scc->ssbSubcarrierSpacing < sizeofArray(nr_slots_per_frame),
              "ssbSubcarrierSpacing %ld is out of range, pointer (%p) might be overwritten, known segfault condition",
