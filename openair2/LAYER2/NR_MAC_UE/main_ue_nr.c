...
void nr_ue_init_mac(NR_UE_MAC_INST_t *mac)
{
  ...
  if (get_nrUE_params()->ntn_koffset || get_nrUE_params()->ntn_ta_common) {
    ...
    if (get_nrUE_params()->ntn_ta_common) {
      ...
      sib19_r17->ntn_Config_r17->ta_Info_r17->ta_Common_r17 = (long)ceil(get_nrUE_params()->ntn_ta_common / 4.072e-6); // ta-Common-r17 is in units of 4.072e-3 µs, ntn_ta_common is in ms
    }
